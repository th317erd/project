...
export class ProjectsCommand {
  constructor(_args) {
    ...
    if (!args.rootPath)
      throw new Error('"rootPath" path not found');

    if (!args.templatePath)
      throw new Error('"templatePath" path not found');

    if (!FileSystem.existsSync(args.rootPath))
      throw new Error(`"rootPath" of [${args.rootPath}] not found`);
  ...
  async walkFiles(path, callback) {
    ...
    for (let i = 0, il = files.length; i < il; i++) {
      ...
      await callback({ rootPath: path, fileName, filePath, stat });
    }
  }
