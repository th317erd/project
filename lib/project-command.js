...
export class ProjectCommand {
  ...
  loadJSON(filePath, skipClean) {
    ...
    let data    = (new Function(`return ${content.trim()};`))();
  ...
  async walkFiles(path, callback) {
    let result = await walk({
      path:         path,           // root dir to start in. defaults to process.cwd()
      ignoreFiles:  [ '.ignore' ],  // list of filenames. defaults to ['.ignore']
    ...
    for (let i = 0, il = result.length; i < il; i++) {
      let relativePath  = result[i];
      let filePath      = Path.join(path, relativePath);
      let stat          = FileSystem.statSync(filePath);

      await callback({ path, fileName: Path.basename(filePath), filePath, stat });
    }
  }
  ...
  async copyFile(scope) {
    ...
    let fileExists      = FileSystem.existsSync(targetFilePath);
    ...
    if (overwriteAction.action === 'merge') {
      console.log('Merging: ', targetFilePath);
      await this.mergeFiles(scope);
    } else {
      if (fileExists)
        console.log('Overwriting: ', targetFilePath);
