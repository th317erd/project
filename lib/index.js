...
const help = {
  ...
  '@examples': [
    './projects init {template-name}',
  ],
};

let args = CMDed(({ $, store, fetch }) => {
  $('--config', Types.STRING(), { format: Path.resolve }) || store('config', Path.join(OS.homedir(), '.config', 'projects', 'config.json'));

  let config = loadConfig(fetch('config'));

  $('--rootPath', Types.STRING(), { format: Path.resolve }) || store('rootPath', process.cwd());
...
if (args) {
  ...
  (async () => {
    try {
      let projectsCommand = new ProjectsCommand({
        config:       args.config,
      ...
      for (let i = 0, il = COMMANDS.length; i < il; i++) {
        ...
        if (options)
          await projectsCommand[commandName](options);
      }
    } catch (error) {
      console.error(error);
    }
